    ...
    def evalvideo(self, net:Yolact, path:str, out_path:str=None):
        ...
        def get_next_frame(vid):
            ...
            for idx in range(self.args.video_multiframe):
                ...
                if frame is None:
                    return frames
                self.actual_frame = cv2.cvtColor(frame, cv2.COLOR_BGR2RGB ) 
                frames.append(frame)
            return frames
            ...
            try:
                ...
                while self.running:
                    ...
                    if not frame_buffer.empty():
                        ...
                        if out_path is None:
                            rgbOut = cv2.cvtColor(frame_buffer.get(), cv2.COLOR_BGR2RGB)
                            qt_masked = self.convert_to_qt(rgbOut)
                            qt_original = self.convert_to_qt(self.actual_frame)
